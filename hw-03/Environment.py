        ...
        for dt in range(tot_time):
            ...
            for agent in self.animal_agents:
                ...
                if agent.ptype == "Prey":
                    ...
                    caught_prey_indices = agent.hunt(self.animal_agents)
                    if caught_prey_indices is not None:
                        temp_hunted_agents.extend(caught_prey_indices)
                    temp_agents.append(agent)

            for index in sorted(temp_hunted_agents, reverse=True):
                del temp_agents[index]
            self.animal_agents.clear
            self.animal_agents = temp_agents
